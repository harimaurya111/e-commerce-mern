                    ...
                    filters.categories.map((category) => (
                        <label key={category} className='cursor-pointer capitalize'>
                            <input type="radio" name='category' id='category' value={category}
                                ...
                                onChange={(e) => setFilterState({ ...filterState, category: e.target.value })} /> <span className='ml-1'>{category}</span> </label>
                    ))
                }
            </div>
                    ...
                    filters.colors.map((color) => (
                        <label key={color} className='cursor-pointer capitalize'>
                            <input type="radio" name='color' id='color' value={color}
                                ...
                                onChange={(e) => setFilterState({ ...filterState, color: e.target.value })} /> <span className='ml-1'>{color}</span> </label>
                    ))
                }
            </div>
                    ...
                    filters.priceRange.map((range) => (
                        <label key={range.label} className='cursor-pointer capitalize'>
                            <input type="radio" name='priceRange' id='priceRange' value={`${range.min}-${range.max}`}
                                ...
                                onChange={(e) => setFilterState({ ...filterState, priceRange: e.target.value })} /> <span className='ml-1'>{range.label}</span> </label>
                    ))
                }
            </div>

            <button onClick={clearFilters} className='bg-primary px-4 py-1 text-white rounded'>Clear all Filters</button>
